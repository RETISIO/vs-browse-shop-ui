...
export default function MobileNavLevelOneItem(props) {
  const [isMobileLevelOneItemOpen, setIsMobileLevelOneItem] = useState(false);
  const [activeLevel2Item, setActiveLevel2Item] = useState('');
    ...
    <div className={mobileMenuClass} id="mobileNav">
      <div role="dialog" aria-modal="true" aria-labelledby="heading-nav" id="dgbox">
        <nav className="navbar-mobile-menu">
          <button className="navbar-toggle js-mobile-menu-toggle mobileNavMenuCloseBtn outline-change first-focusable-element" id="firstFElement" onClick={() => mobileNavCloseHandler()} type="button">
            <span className="sr-only">Toggle navigation</span>
            <i className="icon icon-x fas fa-times outline-change"></i>
          </button>
          <div className={`mobile-menu__level js-mobile-menu-level mobileMenuLevelOne ${mobileMenuLevelOneClass}`} data-level="0">
          ...
          <MobileNavLevelTwoItem
            ...
            isMobileLevelOneItemOpen={isMobileLevelOneItemOpen}
            setIsMobileLevelOneItem={setIsMobileLevelOneItem}
